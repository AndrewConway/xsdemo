...
//      case Some(icon) if icon.contents.lastModified>0 &&  icon.contents.lastModified <= lastMod => NotModified
  ...
  def resultOfMessage(message:ClientMessage) : Result = {
  ...
  def xsComet = Action.async { implicit request =>
        SessionManagement.get(xsCometForm.bindFromRequest.get) match {
          case None => scala.concurrent.Future.successful({ resultOfMessage(ClientMessage.lostSession) })
          case Some(session) =>
            //Async {
